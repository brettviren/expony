#!/usr/bin/env python
'''
An implementation of expony on PyTorch

This implementation differs in some behavior:

- No explicit tile object, a board holds tiles as a 2D integer tensor.
- No copy-on-modify pattern, board state is mutated in-place.

For this instance we do not try to support batched operations.
...
class Board:
    ...
    def __init__(self, source, random_seed=None, device='cpu'):
        '''
        ...
        if isinstance(source, torch.Tensor): # premade
            tiles = source.detach().clone()
            tiles.requires_grad = False
            self.tiles = tiles.to(device=device, dtype=torch.int)
            self.all_positions = positions(self.tiles.shape)
            return

        if isinstance(source, Board): # copy
            tiles = source.tiles.detach().clone()
            tiles.requires_grad = False
            self.tiles = tiles.to(device=device, dtype=torch.int)
            self.rng = source.rng.clone_state()
            self.all_positions = positions(self.tiles.shape)
            return
    ...
    def randint(self, vmin, vmax, shape=1, device='cpu'):
        r = torch.rand(shape, generator = self.rng, requires_grad=False)
        r = vmin + (vmax-vmin) * r
        r = torch.floor(r)
        return r.to(dtype=torch.int, device=device)
    ...
    def set_random(self, pos):
        ...
        self.tiles[pos] = self.randint(1, self.max_init_value,
                                       device=self.tiles.device)

    def swap(self, seed, targ):
        tmp = self.tiles[targ].clone()
        self.tiles[targ] = self.tiles[seed]
        self.tiles[seed] = tmp
